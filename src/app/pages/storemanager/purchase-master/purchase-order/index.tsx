  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await Get(
          "storemanager/purchase-order/list",
          { financialYearId },
          false,
        );
        if (res.data?.success) {
...
export default function PurchaseOrderPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const financialYearId = localStorage.getItem("financialYearId");
        const res = await Get("employee/for-po", { financialYearId }, false);
        if (res.data?.success) {
  ...
  const fetchIndentItems = async (indentId: number | string) => {
    try {
      const res = await Get(
        `storemanager/purchase-order/indent-items/${indentId}`,
        {},
        false,
      );
      if (res.data?.success) {
  ...
  useEffect(() => {
    ...
    const fetchPoNumber = async () => {
      try {
        ...
        const res = await Get(
          "storemanager/purchase-order/next-po-no",
          { financialYearId },
          false,
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await Get(
          "storemanager/purchase-order/next-serial-no",
          { financialYearId },
          false,
        );
  ...
  const fetchSupplierInfo = async (
    ...
    try {
      const res = await Get(
        `storemanager/purchase-order/item-suppliers/${itemId}`,
        {},
        false,
      );
      if (res.data?.success) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await Get("storemanager/itemmaster/purchase-list", {}, false);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const res = await Get("employee/supplier/list", {}, false);
  ...
  const toggleAllMail = () =>
    ...
    try {
      setSubmitting(true);
      const res = await Post("storemanager/purchase-order/create", draftPayload, false);
      if (res.data?.success) {
