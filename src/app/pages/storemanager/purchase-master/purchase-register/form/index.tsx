...
export default function VehiclePurchaseBill() {
  ...
  const handlePoSelect = async (selected: any) => {
    ...
    try {
      const res = await Get(
        `storemanager/purchase-order/${selected.purchaseOrderId || selected.id}`,
        {},
        false,
      );
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await Get("employee/vehicle-list", {}, false);
        if (res.data?.success) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const financialYearId = localStorage.getItem("financialYearId");
        const res = await Get(
          "storemanager/purchase/next-bill-no",
          { financialYearId },
          false,
        );
        if (res.data?.success) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await Get("employee/cash/list", {}, false);
        if (res.data?.success) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await Get("employee/bank/list", {}, false);
        if (res.data?.success) {
  ...
  const validateBeforeSave = (): Record<string, string> => {
    ...
    try {
      ...
      const payload = {
        ...
        createdBy: employeeId ? Number(employeeId) : undefined,
      ...
      const res = await Post("storemanager/purchase/create", payload, false);
      if (res.data?.success) {
        toastsuccessmsg(
