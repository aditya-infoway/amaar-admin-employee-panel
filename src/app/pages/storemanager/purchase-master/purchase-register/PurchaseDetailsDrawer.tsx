...
export function PurchaseDetailsDrawer({ open, purchaseId, onClose }: PurchaseDetailsDrawerProps) {
  ...
  const handleVerifyItem = async (item: PurchaseDetailItem) => {
    ...
    try {
      const res = await Put("storemanager/purchase/item/verify", { purchaseDetailsId: item.purchaseDetailsId }, false);
      if (res.data?.success) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setData(null);
      try {
        const res = await Get(`storemanager/purchase/${purchaseId}`, {}, false);
        if (res.data?.success) {
