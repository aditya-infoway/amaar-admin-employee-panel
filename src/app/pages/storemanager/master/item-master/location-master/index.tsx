...
export default function LocationPage() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const response = await Get("storemanager/location/list", {}, false);
...
const handleSave = async (item: Location) => {
  ...
  try {
    if (item.id) {
      ...
      const response = await Post("storemanager/location/create", payload, false);
      if (response.data?.success) {
        toastsuccessmsg(
