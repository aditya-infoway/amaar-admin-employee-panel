...
export default function ItemGroupPage() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const response = await Get("storemanager/itemgroup/list", {}, false);
 ...
 const handleSave = async (item: ItemGroup) => {
  ...
  try {
    if (item.id) {
      ...
      const response = await Post("storemanager/itemgroup/create", payload, false);
      if (response.data?.success) {
