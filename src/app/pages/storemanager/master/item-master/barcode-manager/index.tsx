...
export default function BarcodeManagerPage() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const res = await Get("storemanager/itemmaster/list", {}, false);
      if (res.data?.success) {
  ...
  const handleManualSave = async (itemId: string) => {
    ...
    try {
      const res = await Put(
        "storemanager/itemmaster/set-barcode",
  ...
  const handleAutoGenerate = async (itemId: string) => {
    ...
    try {
      const res = await Post("storemanager/itemmaster/auto-generate", { itemId: Number(itemId) }, false);
      if (res.data?.success) {
  ...
  const handleBulkGenerate = async () => {
    ...
    try {
      const res = await Post("storemanager/itemmaster/bulk-generate", { itemIds: ids.map(Number) }, false);
      if (res.data?.success) {
