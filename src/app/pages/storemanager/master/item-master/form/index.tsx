...
export default function ItemMasterFormPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await Get("storemanager/itemcategory/list", {}, false);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await Get("storemanager/itemgroup/list", {}, false);
        const list = res?.data?.data || [];
        setAllGroupOptions(
          list.map((g: any) => ({
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res = await Get("storemanager/location/list", {}, false);
        const list = res?.data?.data || [];
  ...
  useEffect(() => {
    if (isEdit && id) {
      (async () => {
        try {
          const res = await Get(`storemanager/itemmaster/${id}`, {}, false);
  ...
  const handleBarcodeTypeChange = async (type: "manual" | "generate") => {
    ...
    if (type === "generate") {
      ...
      try {
        const res = await Get("storemanager/itemmaster/generate-barcode", {}, false);
  ...
  try {
    ...
    if (isEdit && id) {
      res = await Put(
        "storemanager/itemmaster/update",
        {
          itemId: Number(id),
          ...payload,
        },
      ...
      const createPayload: any = { ...payload };
        ...
        createPayload.createdBy = Number(employeeId);
        createPayload.createdType = roleName || "Employee";
     

      res = await Post("storemanager/itemmaster/create", createPayload, false);
