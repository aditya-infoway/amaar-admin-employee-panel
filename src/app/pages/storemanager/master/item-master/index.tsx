...
export default function ItemMasterListPage() {
  ...
  const handleImportExcel = async (file: File) => {
    try {
      ...
      const response = await Post(
        "storemanager/itemmaster/bulk-import",
        { items: importedData },
        false,
      );
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      const response = await Get("storemanager/itemmaster/list", {}, false);
      if (response.data?.success) {
  ...
  const handleDeleteOne = async (row: ItemMaster) => {
    try {
      const response = await Delete(
        "storemanager/itemmaster/delete",
  ...
  const table = useReactTable({
    ...
    meta: {
      openEditDrawer: (row: ItemMaster) =>
        navigate(`/storemanager/item-master/edit/${row.id}`),
